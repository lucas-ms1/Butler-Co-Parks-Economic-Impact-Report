"""
ACS DP02 Education Data Preparation Script
Economic Impact Report for Butler County Parks

This script loads ACS DP02 (Social Characteristics) data from a ZIP file,
extracts tract-level education variables (% high school graduate or higher,
% bachelor's degree or higher), builds a GEOID column, filters to Butler County,
...
# Input path - DP02 ZIP file
dp02_zip_path = os.path.join(
    project_root,
    'data_raw',
    'Health and Census Tracts',
    'ACSDP5Y2023.DP02_2026-01-07T061510.zip'
)

# Output path
output_dir = os.path.join(project_root, 'data_intermediate')
output_path = os.path.join(output_dir, 'butler_acs_dp02_education.csv')

print("="*60)
print("ACS DP02 EDUCATION DATA PREPARATION")
print("="*60)

# Create output directory if it doesn't exist
os.makedirs(output_dir, exist_ok=True)

# 1. Extract and load DP02 data from ZIP
print(f"\n1. Loading ACS DP02 data from ZIP...")
print(f"   Path: {dp02_zip_path}")

# Extract zip to temporary directory
temp_dir = tempfile.mkdtemp()
try:
    with zipfile.ZipFile(dp02_zip_path, 'r') as zip_ref:
    ...
    if data_file is None:
        raise FileNotFoundError(f"Data CSV file not found in ZIP: {dp02_zip_path}")
...
# 4. Select key education variables
print(f"\n4. Selecting key education variables...")
...
variables_to_extract = {
    'GEOID': 'GEOID',
    'NAME': 'NAME',
    'DP02_0067PE': 'pct_high_school_graduate_or_higher',
    'DP02_0068PE': 'pct_bachelors_degree_or_higher'
}
...
numeric_cols = ['pct_high_school_graduate_or_higher', 'pct_bachelors_degree_or_higher']
...
print(f"\n{'='*60}")
print("ACS DP02 EDUCATION DATA PREPARATION COMPLETE")
print(f"{'='*60}")
